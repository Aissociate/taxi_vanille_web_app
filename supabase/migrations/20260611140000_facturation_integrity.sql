/*
  ...
  1. Uniqueness
    - One facture per (chauffeur_id, mois_reference): partial unique index.
      Created only if no duplicates already exist (otherwise a WARNING is
      raised and the index must be created manually after cleanup), so this
      migration never bricks an existing database.

  2. Atomic payment
    - New `payer_facture(p_facture_id)` RPC: locks the facture row, refuses a
      facture already 'payee' (prevents the double-imputation race when two
      payments run concurrently), imputes the advance repayment on
      chauffeur_avances (FIFO, row-locked), then marks the facture 'payee' —
      all in one transaction.
    - SECURITY INVOKER: existing RLS policies on factures / chauffeur_avances
      still apply (back-office users only act on their own rows).

  3. Notes
    - The front-end (FacturationPage) now calls payer_facture() instead of
      doing the imputation client-side.
*/

-- ============================================================
-- 1. One facture per chauffeur per month
-- ============================================================
...
-- ============================================================
-- 2. Atomic payment with advance imputation
-- ============================================================
...
DECLARE
  ...
  v_demande numeric;
BEGIN
  -- Lock the facture row: a concurrent payer_facture() on the same facture
  -- waits here, then fails on the statut check below.
  SELECT * INTO v_fac
  FROM public.factures
